...
function getShizukuImage(current: number, total: number): string {
  ...
  return current % 2 === 0
    ? '/images/characters/minori-smile.png'
...
export default function DiagnosisShell({
  ...
  const share = () => {
    if (!result) return
    const text = `【実家ナビ診断結果】${result.emoji}${result.label}\n${result.headline}\nhttps://jikka-anshin.vercel.app/diagnosis/${diagnosisId}`
  ...
  return (
    <main style={{ maxWidth: 480, margin: '0 auto', padding: '16px 16px 48px' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <Link href="/" style={{ fontSize: 12, color: '#2d6a2d', textDecoration: 'none' }}>
          ← 実家どうするナビ
        </Link>
        <h1 style={{ fontSize: 17, fontWeight: 700, color: '#1a2d1a', margin: '8px 0 2px' }}>{title}</h1>
        <p style={{ fontSize: 12, color: '#2d6a2d', margin: 0 }}>{subtitle}</p>
      </div>

      {!result ? (
        <>
          {/* プログレスバー */}
          <div style={{ background: '#e8f5e8', borderRadius: 99, height: 6, marginBottom: 6 }}>
            <div style={{
              background: 'linear-gradient(90deg, #2d6a2d, #1a5a1a)',
              width: `${progress}%`, height: 6,
              borderRadius: 99, transition: 'width .4s ease'
            }} />
          </div>
          <p style={{ fontSize: 11, color: '#8aaa8a', marginBottom: 16, textAlign: 'right' }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: 12, color: '#3d6a3d', margin: 0, lineHeight: 1.5 }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: 15, fontWeight: 600, color: '#1a2d1a', lineHeight: 1.6, margin: 0 }}>
                  {q?.question}
                </p>
              </div>
            </div>
          ...
          <div style={{ marginLeft: 52 }}>
            {[
              ...
              <button key={i} onClick={() => choose(opt.score!)}
                ...
                <span style={{ color: '#2d6a2d', fontWeight: 700, marginRight: 8 }}>
                  {['A', 'B', 'C'][i]}.
                </span>
                {opt.label}
              </button>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, color: '#2d6a2d', fontWeight: 700, marginBottom: 6 }}>
              あなたの診断結果
            </div>
            <div style={{ fontSize: 20, fontWeight: 800, color: '#1a2d1a', marginBottom: 8 }}>
              {result.label}
            </div>
            <p style={{ fontSize: 14, color: '#1a4a1a', lineHeight: 1.6, margin: 0 }}>
              {result.headline}
            </p>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[result.feature1, result.feature2, result.feature3].map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  <span style={{ color: '#2d6a2d', fontSize: 14, flexShrink: 0 }}>✓</span>
          ...
          {roadmap && (
            <Link href={`/roadmap/${result.roadmapSlug}`}
              style={{
                display: 'block',
                background: 'linear-gradient(135deg, #1a5a1a, #2d6a2d)',
            ...
            <div style={{ background: '#fff', border: '1px solid #b8ddb8', borderRadius: 12, padding: '16px', marginBottom: 12 }}>
              ...
              {recommendServices.map((s, i) => (
                <a key={i} href={s.url} target="_blank" rel="nofollow noopener noreferrer"
                  style={{
                    display: 'block', border: '1px solid #b8ddb8',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, background: '#f0faf0', color: '#7b52c8', borderRadius: 4, padding: '2px 6px' }}>
                      {s.badge}
                    </span>
                  </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #b8ddb8', borderRadius: 12, padding: '16px', marginBottom: 12 }}>
              ...
              {nextDiagnoses.map((d, i) => (
                <Link key={i} href={d.href}
                  ...
                  <span style={{ fontSize: 12, color: '#2d6a2d' }}>→</span>
                </Link>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={share} style={{
              ...
              background: '#fff', color: '#1a5a1a', fontWeight: 600,
            }}>
              シェアする
            </button>
